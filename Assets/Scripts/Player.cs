...
public class Player : MonoBehaviour
{
  ...
  private IEnumerator tripleShotCoroutine, speedCoroutine;

  // 3 bools that tell this class when another of the same powerup has been collected so we can extend the duration of that powerup in Update()
  private bool _isAnotherTripleShot = false;
  ...
  void Update()
  {
    ...
    // extend the duration of powerup if another of the same type was collected
    if (_isAnotherSpeedBoost == true)
    {
      StopCoroutine(speedCoroutine); // stop the current coroutine
      _speed /= 2;  // reduce the speed (like in the coroutine since we stopped it)
      SpeedBoostActive(); // Activate the power up again (which will start a new coroutine (see this method)) to power down 
      _isAnotherSpeedBoost = false; // set to false or else infinte reduction in speed
    }
